...
router.post('/', auth, async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      `INSERT INTO lotes (codigo, data_compra, codigo_material, peso_comprado, custo_total, observacao)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [codigo.trim().toUpperCase(), data_compra, codigo_material.trim().toUpperCase(),
       Number(peso_comprado), Number(custo_total), observacao?.trim() || null]
    );
  ...
  try {
    const { rows } = await db.query(
      `UPDATE lotes SET codigo=$1, data_compra=$2, codigo_material=$3,
       peso_comprado=$4, custo_total=$5, observacao=$6 WHERE id=$7 RETURNING *`,
      [codigo.trim().toUpperCase(), data_compra, codigo_material.trim().toUpperCase(),
       Number(peso_comprado), Number(custo_total), observacao?.trim() || null, req.params.id]
    );
...
router.delete('/:id', auth, async (req, res) => {
  try {
    await db.query('DELETE FROM lotes WHERE id = $1', [req.params.id]);
